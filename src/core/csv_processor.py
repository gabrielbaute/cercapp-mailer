...
class CSVProcessor:
    ...
    def read_recipients(self, csv_file: str) -> List[Dict[str, str]]:
        ...
        try:
            with open(csv_file, mode='r', encoding='utf-8') as file:
                self.logger.info(f"[{self.module_name}]: Abriendo archivo CSV: {csv_file}")
                ...
                if not all(f in fieldnames for f in ['email', 'name']):
                    self.logger.error(f"[{self.module_name}]: El CSV debe contener columnas 'email' y 'name'")
                    return recipients
                ...
                for row in reader:
                    ...
                    # Validaciones
                    if not email:
                        self.logger.error(f"[{self.module_name}]: Email vacío - Fila: {row}")
                        continue
                        
                    if not self.mailer.is_valid_email(email):
                        self.logger.error(f"[{self.module_name}]: Email no válido '{email}' - Fila: {row}")
                        continue
                        
                    if not name:
                        self.logger.warning(f"[{self.module_name}]: Nombre vacío para email {email} - Usando valor por defecto")
                        name = "Estimado/a"
                    
                    recipients.append({'email': email, 'name': name})
                
                self.logger.info(f"[{self.module_name}]: Total de contactos válidos leídos: {len(recipients)}")
        
        except Exception as e:
            self.logger.error(f"[{self.module_name}]: Error leyendo el archivo CSV: {str(e)}", exc_info=True)
    ...
    def process_contacts(self, csv_file: str, template_name: str, subject: str) -> Dict[str, int]:
        ...
        for contact in recipients:
            ...
            if email:
                self.logger.info(f"[{self.module_name}]: Enviando email a {email}")
                try:
                    ...
                    self.logger.error(f"Error crítico enviando email a {email}: {str(e)}", exc_info=True)
                    result['failures'] += 1
