...
from src.mail.token_handler import MailTokenHandler

class Mailer:
    def __init__(self, app=None):
        self.mail = Mail()
        self.module_name = "BackendMailer"
        if app:
            self.init_app(app)
    ...
    def init_app(self, app):
        ...
        try:
            ...
            current_app.logger.debug(f"[{self.module_name}]: Inicializando Flask-Mail")
        except Exception as e:
            current_app.logger.error(f"[{self.module_name}]: Error al inicializar Flask-Mail: {e}")

    def render_template(self, template_name, **context):
        """Renderiza una plantilla Jinja2 con contexto"""
        try:
            template = self.env.get_template(template_name)
            current_app.logger.debug(f"[{self.module_name}]: Plantilla renderizada: {template}")
            return template.render(**context)
        except Exception as e:
            current_app.logger.error(f"[{self.module_name}]: Error al renderizar plantilla: {e}")
            return None
    ...
    def send_email(self, subject, recipients, template_name, context, sender=None):
        ...
        current_app.logger.debug(f"[{self.module_name}]: Sender: {sender}")
        ...
        try:
            ...
            current_app.logger.debug(f"[{self.module_name}]: Correo enviado: {msg}")
            self.mail.send(msg)
        except Exception as e:
            current_app.logger.error(f"[{self.module_name}]: Error al enviar correo: {e}")
